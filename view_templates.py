...
def get_retronotes_list_view(retronotes, response_type=DEFAULT_VIEW_RESPONSE_TYPE, replace_original=DEFAULT_VIEW_REPLACE_ORIGINAL):
    return {
        "response_type": response_type,
        "replace_original": replace_original,
    	"blocks": [
    		...
    		},
    		{
    			...
    			"text": {
    				"type": "mrkdwn",
    				"text": "_Добавить заметку можно с помощью команды `/retronote add` или с помощью кнопки *{}* ниже_".format(ADD_NOTE_BUTTON_NAME)
    			}
    		},
            {
                "type": "actions",
...
def get_simple_message_view(text, response_type=DEFAULT_VIEW_RESPONSE_TYPE, replace_original=DEFAULT_VIEW_REPLACE_ORIGINAL):
    return {
        "response_type": response_type,
        "replace_original": replace_original,
        "text": text
